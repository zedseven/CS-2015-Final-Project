...
while running:
    ...
    ##                print("modded2: " + str((int(newPoint[0][1:len(newPoint[0])]),int(newPoint[1][1:-1]))))
                    trailPositions.append((int(newPoint[0][1:len(newPoint[0])]),int(newPoint[1][1:-1]))) #ADD REMOVABILITY
                else:
...
##                lPower.rect.x = (((width - lPower.rect.width) - (edgesPadding * 2)) * random.random()) + (edgesPadding * 1) #I am aware * 1 is useless, but is allows for me to see only one is added
##                lPower.rect.y = ((((height - screenOffset) - lPower.rect.height) - (edgesPadding * 2)) * random.random()) + (screenOffset + (edgesPadding * 1))
                spawnPowerUp(lPower)
...
##                sPower.rect.y = (((height - screenOffset) - sPower.rect.height) - (edgesPadding * 2)) * random.random()
        ...
        if nextHPowerUpSpawn <= currentScore:
            ...
            edgesPadding = 30
            while spawnedOnPlayer == True:
##                hPower.rect.x = ((width - hPower.rect.width) - (edgesPadding * 2)) * random.random()
##                hPower.rect.y = (((height - screenOffset) - hPower.rect.height) - (edgesPadding * 2)) * random.random()
                spawnPowerUp(hPower)
        ...
        if savedHighscore == False:
            ...
            for index in range(len(highscoresList)):
                #inversedIndex = (len(highscoresList) - 1) - index
                currentLine = highscoresList[index] #inversedIndex]
                ...
                if int(str(currentHighScore).strip()) < currentScore and newScore == False:
                    ...
                    newHighscoresList.insert(index, currentDate + " " + str(currentScore))#inversedIndex
            ...
            with open(highscoresFile,"r+") as output:
                ...
                for lineIndex in range(len(newHighscoresList)):
                    if lineIndex < numScores:
                        ...
                        if lineIndex < (numScores - 1):#!= len(newHighscoresList) - 1:
                            text += "\n"
                output.seek(0)
                output.write(text)
                output.truncate()
